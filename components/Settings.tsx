...
import { PropertyAddressLookup } from './address/PropertyAddressLookup';
    ...
    <div className="flex flex-col md:flex-row gap-6">
      ...
      <div className="flex-1 glass-panel rounded-3xl p-6 md:p-8">
        {tab === 'profile' && <div className="max-w-2xl space-y-5">
          <div className="flex items-center gap-4"><Avatar size="xl" name={profile.fullname || [profile.firstName, profile.lastName].filter(Boolean).join(' ') || email || 'Administrator'} source={profile.profileUrl} /><div><h3 className="font-bold text-xl">{profile.fullname || [profile.firstName, profile.lastName].filter(Boolean).join(' ') || 'Administrator'}</h3><p className="text-sm text-gray-500">Administrator</p></div></div>
          <div className="grid md:grid-cols-2 gap-4"><label className="text-xs font-bold text-gray-600">FIRST NAME<input value={profile.firstName || ''} onChange={(e) => setProfile({...profile, firstName:e.target.value})} className="glass-input mt-2 w-full p-3 rounded-xl text-sm" /></label><label className="text-xs font-bold text-gray-600">LAST NAME<input value={profile.lastName || ''} onChange={(e) => setProfile({...profile, lastName:e.target.value})} className="glass-input mt-2 w-full p-3 rounded-xl text-sm" /></label><label className="md:col-span-2 text-xs font-bold text-gray-600">EMAIL<input value={email} readOnly className="glass-input mt-2 w-full p-3 rounded-xl text-sm bg-gray-100" /><span className="block mt-1 normal-case font-normal text-gray-500">Email changes are not supported by the existing profile API.</span></label><label className="md:col-span-2 text-xs font-bold text-gray-600">ROLE TITLE<input value={profile.title || ''} onChange={(e) => setProfile({...profile,title:e.target.value})} className="glass-input mt-2 w-full p-3 rounded-xl text-sm" /></label><label className="md:col-span-2 text-xs font-bold text-gray-600">HOME ADDRESS<PropertyAddressLookup id="admin-address" value={profile.address || ''} country={profile.country} city={profile.city || undefined} className="glass-input mt-2 w-full p-3 rounded-xl text-sm" onChange={(address) => setProfile({...profile, address})} onResolved={(parts) => setProfile((old) => ({...old, address: parts.address, city: parts.city, state: parts.state, zip: parts.postcode, country: parts.country || old.country}))} /><span className="block mt-1 normal-case font-normal text-gray-500">Start typing a street or postcode. City, region and postcode fill from the same address book as landlord and tenant.</span></label><label className="text-xs font-bold text-gray-600">CITY<input value={profile.city || ''} onChange={(e) => setProfile({...profile, city:e.target.value})} className="glass-input mt-2 w-full p-3 rounded-xl text-sm" /></label><label className="text-xs font-bold text-gray-600">COUNTY / STATE<input value={profile.state || ''} onChange={(e) => setProfile({...profile, state:e.target.value})} className="glass-input mt-2 w-full p-3 rounded-xl text-sm" /></label><label className="text-xs font-bold text-gray-600">POSTCODE<input value={profile.zip || ''} onChange={(e) => setProfile({...profile, zip:e.target.value})} className="glass-input mt-2 w-full p-3 rounded-xl text-sm" /></label><label className="text-xs font-bold text-gray-600">COUNTRY<input value={profile.country || ''} onChange={(e) => setProfile({...profile, country:e.target.value})} className="glass-input mt-2 w-full p-3 rounded-xl text-sm" /></label></div>
          <button disabled={saving} onClick={() => runSave(async () => { const p = await updateMyProfile({firstName:profile.firstName,lastName:profile.lastName,title:profile.title,address:profile.address,city:profile.city,state:profile.state,zip:profile.zip,country:profile.country}); setProfile((old) => ({...old,...p})); }, 'Profile saved.')} className="bg-red-600 text-white px-6 py-2.5 rounded-xl font-bold disabled:opacity-50">{saving?'Saving…':'Save Profile'}</button>
        </div>}
